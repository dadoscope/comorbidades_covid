...
for(i in 1:length(pacientes)){
   for(j in 1:length(comorbidades)){
...
degreeIn[unique(sort(MisJson$links$target))+1] <- as.numeric(table(MisJson$links$target))[order(as.character(table(MisJson$links$target)))]
#degreeOut
#degreeIn
degreeOut <- rep(0, nrow(MisJson$nodes))
degreeOut[unique(sort(MisJson$links$source))+1] <- as.numeric(table(MisJson$links$source))[order(as.character(table(MisJson$links$source)))]
#size
